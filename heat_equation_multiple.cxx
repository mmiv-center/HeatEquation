      ...
      for (int i = 1; i < dims[0] - 1; i++) {
        ...
        if (data[count] != 0 || zeroSpecified) { // do something (not exterior)
          ...
          if (temperatureByMaterial.find(data[count]) != temperatureByMaterial.end()) { // if the temperature is set, do not change it
            ...
            // we do a transfer function to adjust the amount of stuff getting in, based on the current temperature
            double hereTemp = ((val111 - (-100.0)) / (200.0)) - 0.5; // map to -0.5..0.5
            double incomingTemp = (val101 + val121 + val011 + val211 + val110 + val112);
            double transfer = 0.0; // 4.0f * pow(hereTemp, 3.0); // 1. / (1.0 + 1.0 * pow(exp(-hereTemp), 12)) - 0.5;
            if (fabs(hereTemp) < 0.001)
              transfer = 0;
            else
              transfer = hereTemp;
            double newHereTemp = ((transfer + 0.5) * 200.0) + (-100.0);
            result = (1.0 - 6.0 * omega) * newHereTemp + omega * (val101 + val121 + val011 + val211 + val110 + val112);
...
int main(int argc, char *argv[]) {
  ...
  command.SetDescription("Simulation of the heat equation - with some twists. Use as in: ./HeatEquation -s 2 -n -q 3 -i 2000 wm.seg.nii /tmp/ "
